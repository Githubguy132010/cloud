...
if (dbConnected) {
  ...
  // Auth check (GET /api/admin/storage should return 410, not 401/403)
  const { status: authCode } = await jwtGet('/api/admin/storage');
  assertEq('Auth check returns 410 (not 401/403)', 410, authCode);
