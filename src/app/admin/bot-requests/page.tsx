...
const breadcrumbs = (
  <>
    <BreadcrumbItem>
      <BreadcrumbPage>Bot Requests</BreadcrumbPage>
    </BreadcrumbItem>
  </>
);
...
function toSortedSearchParams(params: Record<string, string>): string {
...
export default function BotRequestsPage() {
  ...
  const pushWith = useCallback(
    (overrides: Record<string, string>) => {
      ...
      router.push(`/admin/bot-requests?${toSortedSearchParams(next)}`);
    },
    [searchParams, router]
  );
    ...
    <AdminPage breadcrumbs={breadcrumbs}>
      <div className="flex w-full flex-col gap-y-6">
        ...
        <RequestsTable
          ...
          onLimitChange={l => pushWith({ limit: String(l), page: '1' })}
        />
      </div>
    </AdminPage>
  );
